    ...
    <div className="relative min-h-screen overflow-x-hidden bg-[#020617] text-white">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 pb-16 pt-6 sm:px-6">
        <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3 rounded-full border border-white/15 bg-black/40 px-4 py-2 shadow-lg backdrop-blur-md">
            <Image src={MEDIA_ASSETS.logoPnc} alt="PNC" width={40} height={40} className="h-9 w-9 object-contain" />
            <Image src={MEDIA_ASSETS.logoJurusan} alt="JKB" width={40} height={40} className="h-9 w-9 object-contain" />
            <Image src={MEDIA_ASSETS.logoHimatris} alt="HIMATRIS" width={40} height={40} className="h-9 w-9 object-contain" />
          </div>
        ...
        <div className="mb-8 text-center">
          <p className="text-sm font-medium uppercase tracking-[0.25em] text-sky-300/90">
          ...
          <p className="mt-1 text-2xl font-extrabold tracking-wide sm:text-4xl">
        ...
        {loading ? (
          ...
          <>
            <div className="grid gap-6 lg:grid-cols-2">
              {rows.map((row, index) => {
                ...
                return (
                  <article
                    key={row.paslonId}
                    className={`overflow-hidden rounded-3xl border bg-black/50 shadow-2xl backdrop-blur-md ${
                      isLeader
                  ...
                  >
                    <div className="relative aspect-[4/3] w-full overflow-hidden bg-slate-900">
                      ...
                      <div className="absolute left-4 top-4 flex flex-wrap gap-2">
                        <span className="rounded-full bg-gradient-to-r from-amber-300 to-yellow-400 px-3 py-1 text-xs font-black uppercase text-slate-900">
                          Paslon {row.nomor}
                        </span>
                        {isLeader && (
                          <span className="rounded-full bg-cyan-400 px-3 py-1 text-xs font-bold uppercase text-slate-900">
                            Terdepan
                          </span>
                        )}
                      </div>
                    ...
                    <div className="space-y-4 p-5">
                      <div className="text-center">
                        ...
                        <p
                          className={`bg-gradient-to-r ${accent.bar} bg-clip-text text-5xl font-black tabular-nums text-transparent`}
                        >
                          {percent}%
                        </p>
                      </div>
            ...
            <section className="mt-8 grid gap-6 lg:grid-cols-2">
              <div className="rounded-3xl border border-white/10 bg-black/50 p-5 backdrop-blur-md">
      ...
      {visiMisi && (
        ...
        >
          <div
            className="relative max-h-[90vh] w-full max-w-3xl overflow-hidden rounded-2xl border border-white/15 bg-slate-950 shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
              <h3 className="font-semibold text-sky-100">{visiMisi.title}</h3>
              <button
                type="button"
                onClick={() => setVisiMisi(null)}
            ...
            <div className="max-h-[calc(90vh-3.5rem)] overflow-y-auto">
